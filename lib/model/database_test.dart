...
class DatabaseTest {
  ...
  static Future<void> addItem({
    ...
    await documentReferencer
        ...
        .whenComplete(() => debugPrint(
            "Event $title have been added into database avec id " + docIdAdd!))
  ...
  static Future<void> updateItem({
    ...
    await documentReferencer
        .update(data)
        .whenComplete(() => debugPrint("DB changed $userUid"))
    ...
    for (var i = 0; i < dataSync.size; i++) {
      ...
      await _mainCollection
          ...
          .doc(docId)
          .update(data)
          .whenComplete(() => debugPrint("DB changed $email"))
          .catchError((e) => debugPrint(e));
    }
  }
  ...
  static void fetchDataID() async {
    ...
    debugPrint(
        "length : " + listRole.length.toString() + " " + listRole.toString());
  }

  /*---------------------------------------*/
  //mettre à jour la quantité de chaque rôle
  ...
  static Future<bool> fetchDataCheckUpdateDB(
    ...
    for (int i = 0; i < dataNbEnter.docs.length; i++) {
      ...
      if (!email.contains(userUid) &&
          (role.contains("Organisateur") || role.contains("Scanneur")))
        lstEmailClient.add(email);
    }
        ...
        if (!lstPersonScanned.containsKey(contentQRCode)) {
          ...
          for (int j = 0; j < lstEmailClient.length; j++) {
            _mainCollection
                ...
                .whenComplete(() => debugPrint(
                    "${lstEmailClient[j]} Updated $emailClient status: true"))
                .catchError((e) => debugPrint(e));
          }
  ...
  static Future<void> updateGroup(
    ...
    hashMapGrChanged.keys.toList().forEach((element) {
      hmGrUpdated.updateAll((key, value) =>
          value.replaceAll(element, hashMapGrChanged[element]!));
    });
    ...
    for (int i = 0; i < hmGrUpdated.length; i++) {
      ...
      Map<String, dynamic> updateNameGr = <String, dynamic>{"group": name};
    ...
    for (int i = 0; i < hmEmailUpdated.length; i++) {
      ...
      DocumentReference updatedTime =
          _mainCollection.doc(email).collection(eventRelated).doc(docId);
